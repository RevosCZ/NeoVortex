...
@onready var honor = $Honor
@onready var treats = $Treats
@onready var zoomies = $Zoomies
...
func _process(delta):
	...
	controls.text = "Hold LMB to select | Click RMB to move selected units | Click LMB on CatBox | S to increase speed (L1: 100P L2: 200P)"
